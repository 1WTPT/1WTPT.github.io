...
namespace JournalTrace.View.Layout
{
    public partial class TreeLayout : UserControl, ILayout
    {
        ...
        public async void LoadData(FormMain frm)
        {
            ...
            ApplyTreeNodeColors(treeUSNDirectories.Nodes);

            frm.ShowLayoutOption(true);
        }

        private static readonly Color TreeIdentifierColor = Color.FromArgb(168, 160, 181);
        private static readonly Color TreeNameColor = Color.FromArgb(245, 242, 250);

        private void ApplyTreeNodeColors(TreeNodeCollection nodes)
        {
            foreach (TreeNode node in nodes)
            {
                ulong numericValue;
                bool isIdentifier = node.Text.EndsWith(":") || ulong.TryParse(node.Text, out numericValue);
                node.ForeColor = isIdentifier ? TreeIdentifierColor : TreeNameColor;
